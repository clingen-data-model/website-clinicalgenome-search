...
function symbolHgncFormatter(index, row) {
  var html = '<span onclick="event.stopPropagation();" ><a href="/kb/genes/' + row.hgnc_id + '"><b>' + row.symbol + '</b></a></span>' + '<div class="text-muted small">' + row.hgnc_id + '</div>';
  return html;
}
...
function zeroFormatter(index, row) {
  if (row.total_secondary_curations == 0) return "-";
...
function reportableFormatter(index, row) {
  if (row.reportable) return '<span onclick="event.stopPropagation();"><i class="fas fa-check"></i></span>';else return '';
}
...
function location02Formatter(index, row) {
  //if (row.type == 0)
  //   return row.location;
  if (row.coordinates == null) return '';
  var name = row.coordinates.trim(); // strip off chr
...
function locationFormatter(index, row) {
  //if (row.type == 0)
  //   return row.location;
  if (row.grch37 == null) return '<div class="text-warning text-center" data-toggle="tooltip" data-placement="top" title="This gene has no GRCh37 genomic coordinates."><i class="fas fa-exclamation-triangle"></i></div>';
...
function location38Formatter(index, row) {
  //if (row.type == 0)
  //   return row.location;
  if (row.grch38 == null) return '<div class="text-warning text-center" data-toggle="tooltip" data-placement="top" title="This gene has no GRCh38 genomic coordinates."><i class="fas fa-exclamation-triangle"></i></div>';
...
function plofFormatter(index, row) {
  ...
  if (row.plof < .6) return '<span class="format-pli-high">' + row.plof + '</span>';else return '<span class="format-pli-low">' + row.plof + '</span>';
}
...
function haplo2Formatter(index, row) {
  if (row.haplo_assertion === null || row.haplo_assertion === false) return '';
  if (row.haplo_assertion == 'Not yet evaluated' || row.haplo_assertion == "-5") return '';
  var display = '';
  var tooltip = '';
  ...
  if (row.haplo_history === null) return html; // tack on the change score icon

  html += '<span class="pointer text-danger mt-2" data-toggle="tooltip" data-placement="top" title="' + row.haplo_history + '"><i class="far fa-clock ml-2 fa-lg"></i></span>';
  return html;
}
...
function triplo2Formatter(index, row) {
  if (row.triplo_assertion === null || row.triplo_assertion === false) return '';
  if (row.triplo_assertion == 'Not yet evaluated' || row.triplo_assertion == "-5") return '';
  var display = '';
  var tooltip = '';
  ...
  var html = '<span onclick="event.stopPropagation();" class="small badge cg-' + badge + ' p-2" data-toggle="tooltip" data-placement="top" title="' + tooltip + '"><a class="text-white" href="/kb/gene-dosage/' + (row.type == 0 ? '' : 'region/') + row.symbol_id + '" target="_gt">' + display + '</a></span>';
  if (row.triplo_history === null) return html; // tack on the change score icon

  html += '<span class="pointer text-danger mt-2" data-toggle="tooltip" data-placement="top" title="' + row.triplo_history + '"><i class="far fa-clock ml-2 fa-lg"></i></span>';
  return html;
}
...
function omimcomboFormatter(index, row) {
  ...
  switch (row.omimcombo) {
    ...
    case 1:
      return '<span onclick="event.stopPropagation();" ><a href="https://omim.org/entry/' + row.omimlink + '" > <span class="text-dark">OMIM</span></a></span>' + '<hr class="mt-1 mb-1 mr-4"><div>&nbsp;</div>';

    case 2:
      return '<span>&nbsp;</span>' + '<hr class="mt-1 mb-1 mr-4"><div onclick="event.stopPropagation();" ><a href="https://omim.org/entry/' + row.omimlink + '" > <span class="text-dark">Morbid</div></a></span>';

    case 3:
      return '<span onclick="event.stopPropagation();" ><a href="https://omim.org/entry/' + row.omimlink + '" > <span class="text-dark">OMIM</span></a></span>' + '<hr class="mt-1 mb-1 mr-4"><div onclick="event.stopPropagation();" ><a href="https://omim.org/entry/' + row.omimlink + '" > <span class="text-dark">Morbid</div></a></span>';
  }
...
function omimFormatter(index, row) {
  var html = '';
  if (row.omimlink) html = '<span onclick="event.stopPropagation();" ><a href="https://omim.org/entry/' + row.omimlink + '" > <span class="text-dark">OMIM</span></a></span>';else html = '<span>&nbsp;</span>'; //html = '<span onclick="event.stopPropagation();" ><a href="https://omim.org/entry/' + row.omimlink + '" > <span class="text-success"><i class="fas fa-check"></i></span></a></span>';

  if (row.morbid == "Yes") html += '<hr class="mt-1 mb-1 mr-4"><div onclick="event.stopPropagation();" ><a href="https://omim.org/entry/' + row.omimlink + '" > <span class="text-dark">Morbid</div></a></span>';
  return html;
  ;
}
...
function reportFormatter(index, row) {
  ...
  if (row.type == 0 || row.type == 3) {
    ...
    return '<span onclick="event.stopPropagation();" ><a class="btn btn-sm btn-outline-dark btn-block rounded-0" href="' + '/kb/gene-dosage/' + row.hgnc_id + '"><i class="fas fa-file mr-2"></i>   ' + row.date + '</a></span>';
  } else {
    return '<span onclick="event.stopPropagation();" ><a class="btn btn-sm btn-outline-dark btn-block rounded-0" href="' + '/kb/gene-dosage/region/' + row.hgnc_id + '"><i class="fas fa-file mr-2"></i>   ' + row.date + '</a></span>';
  }
}
...
function affiliateFormatter(index, row) {
  ...
  //return html;
  var html = '<a onclick="event.stopPropagation();" href="https://clinicalgenome.org/affiliation/' + row.agent + '" target="_gcep">' + row.label + '</a>' + '<div class="text-muted small">ClinGen Affiliate ID: ' + row.agent + '</div>';
  return html;
}
...
function badgeFormatter(index, row) {
  var html = '';
  if (row.has_validity) html += '<img class="" src="/images/clinicalValidity-on.png" title="Gene-Disease Validity" style="width:30px">';else html += '<img class="" src="/images/clinicalValidity-off.png" title="Gene-Disease Validity" style="width:30px">';
  if (row.has_dosage) html += '<img class="" src="/images/dosageSensitivity-on.png" title="Dosage Sensitivity" style="width:30px">';else html += '<img class="" src="/images/dosageSensitivity-off.png" title="Dosage Sensitivity" style="width:30px">';
  if (row.has_actionability) html += '<img class="" src="/images/clinicalActionability-on.png" title="Clinical Actionability" style="width:30px">';else html += '<img class="" src="/images/clinicalActionability-off.png" title="Clinical Actionability" style="width:30px">';
  if (row.has_variant) html += '<img class="" src="/images/variantPathogenicity-on.png" title="Variant Pathogenicity" style="width:30px">';else html += '<img class="" src="/images/variantPathogenicity-off.png" title="Variant Pathogenicity" style="width:30px">';
  if (row.has_pharma) html += '<img class="" src="/images/Pharmacogenomics-on.png" title="Pharmacogenomics" style="width:30px">';else html += '<img class="" src="/images/Pharmacogenomics-off.png" title="Pharmacogenomics" style="width:30px">';
  return html;
}

function badge2Formatter(index, row) {
  var html = '';
  if (row.has_validity) html += '<img class="" src="/images/clinicalValidity-on.png" title="Gene-Disease Validity" style="width:30px">';
  if (row.has_dosage) html += '<img class="" src="/images/dosageSensitivity-on.png" title="Dosage Sensitivity" style="width:30px">';
  if (row.has_actionability) html += '<img class="" src="/images/clinicalActionability-on.png" title="Clinical Actionability" style="width:30px">';
  if (row.has_variant) html += '<img class="" src="/images/variantPathogenicity-on.png" title="Variant Pathogenicity" style="width:30px">';
  if (row.has_pharma) html += '<img class="" src="/images/Pharmacogenomics-on.png" title="Pharmacogenomics" style="width:30px">';
  if (row.date_last_curated) html += '<div class="text-muted small">Last Curated: ' + row.date_last_curated + '</div>';
  return html;
}

function readMoreFormatter(index, row) {
  if (row.comments) return '<span class="add-read-more show-less-content">' + row.comments + '</span>';else return '<span class="text-muted text-center font-italic mt-4 ml-5">There are no guidance notes at this time.</span>';
}

function diseaseCountFormatter(index, row) {
  var html = '';
  if (row.has_validity) html += '<img class="" src="/images/clinicalValidity-on.png" title="Gene-Disease Validity" style="width:30px">';else html += '<img class="" src="/images/clinicalValidity-off.png" title="Gene-Disease Validity" style="width:30px">';
  if (row.has_dosage) html += '<img class="ml-2" src="/images/dosageSensitivity-on.png" title="Dosage Sensitivity" style="width:30px">';else html += '<img class="ml-2" src="/images/dosageSensitivity-off.png" title="Dosage Sensitivity" style="width:30px">';
  if (row.has_actionability) html += '<img class="ml-2" src="/images/clinicalActionability-on.png" title="Clinical Actionability" style="width:30px">';else html += '<img class="ml-2" src="/images/clinicalActionability-off.png" title="Clinical Actionability" style="width:30px">';
  if (row.has_variant) html += '<img class="ml-2" src="/images/variantPathogenicity-on.png" title="Variant Pathogenicity" style="width:30px">';else html += '<img class="ml-2" src="/images/variantPathogenicity-off.png" title="Variant Pathogenicity" style="width:30px">';
  return html;
}
...
function variantGuidanceFormatter(index, row) {
  ...
  if (row.has_comment) html += ' and reporting guidance';
...
function reportingGuidanceFormatter(index, row) {
  var html = '';
  if (row.has_comment) html += '  <h5 class="text-danger">Reporting<br>Guidance<br>Available</h5>';
  return html;
}
...
function asbadgeFormatter(index, row) {
  ...
  if (row.animal_model_only) return '<a class="btn btn-default btn-block btn-classification" onclick="event.stopPropagation();" href="/kb/gene-validity/' + row.perm_id + '">' + '' + txt + '<div class="badge badge-warning">' + 'Animal Model Only</div></a>'; //return '<a class="btn btn-default btn-block btn-classification" href="/kb/gene-validity/' + row.perm_id + '">'
  //    + '' + txt + '</a>';

  return '<a onclick="event.stopPropagation();" class="btn btn-block text-white cg-table-' + color + '" href="/kb/gene-validity/' + row.perm_id + '">' + '' + txt + '</a>';
}

function datebadgeFormatter(index, row) {
  return '<a onclick="event.stopPropagation();" class="btn btn-sm btn-outline-dark btn-block rounded-0" href="/kb/gene-validity/' + row.perm_id + '"><i class="far fa-file mr-2"></i> ' + '' + row.released + '</a>';
}

function contributorbadgeFormatter(index, row) {
  return row.contributor_type; // return '<span class="badge badge-light border-1 text-left font-weight-normal">'
...
function conditionFormatter(index, row) {
  ...
  html += '</div>';
  if (row.synonym != null) html += '<div class="text-sm text-muted">Synonym: ' + row.synonym + '</div>';
  return html;
}
...
function hasvalidityFormatter(index, row) {
  ...
  return '<a onclick="event.stopPropagation();" class="btn btn-success btn-sm pb-0 pt-0" href="/kb/genes/' + row.hgnc_id + '"><i class="glyphicon glyphicon-file"></i> <span class="hidden-sm hidden-xs">Curated</span></a>';
}
...
function hasdosageFormatter(index, row) {
  ...
  return '<a onclick="event.stopPropagation();" class="btn btn-success  btn-wrap btn-sm pb-0 pt-0" href="/kb/genes/' + row.hgnc_id + '"><i class="glyphicon glyphicon-file"></i> <span class="hidden-sm hidden-xs">Curated</span></a>';
...
function cnvreportFormatter(index, row) {
  if (row.rawdate === "") return '<span onclick="event.stopPropagation();" ><a class="btn btn-sm btn-outline-dark btn-block rounded-0" href="/kb/gene-dosage/region/' + row.key + '"><i class="fas fa-file"></i>  Under Review</a></span>';
  return '<span onclick="event.stopPropagation();" ><a class="btn btn-sm btn-outline-dark btn-block rounded-0" href="/kb/gene-dosage/region/' + row.key + '"><i class="fas fa-file"></i>   ' + row.date + '</a></span>';
}
...
function acmglinkFormatter(index, row) {
  ...
  if (row.has_variant) html += "<div onclick='event.stopPropagation();'><a href='" + row.cspec_link + "' target='_cspec'><b>CSpec Registry <i class='fas fa-external-link-alt'></i></b></a></div>";else html += '<div onclick="event.stopPropagation();">&nbsp;</div>';
  return html;
}
...
function precurationFormatter(index, row) {
  if (row.locus_type == 'pseudogene') return '<div class="text-center text-muted font-weight-bold reduced-line-height border-warning border-1 mr-1 pt-1 pb-1">' + "Pseudogenes not reviewable by Dosage Sensitivity" + '</div>';
...
function symbol2Formatter(index, row) {
  if (typeof row.type == 'undefined' || row.type == 0) return '<span onclick="event.stopPropagation();" ><a href="/kb/genes/' + row.symbol_id + '"><b>' + row.symbol + '</b></a></span>' + '<div class="text-muted small">' + row.symbol_id + '</div>';
  if (row.type == 1) return '<span onclick="event.stopPropagation();" ><a href="/kb/gene-dosage/region/' + row.symbol_id + '"><b>' + row.symbol + '</b></a></span>';
  return "bad";
}
...
function relationFormatter(index, row) {
  ...
  if (row.type == 0) {
    if (row.locus_type == "pseudogene") html += '<div class="text-left text-warning font-weight-bold mb-1" title="Pseudogene">Pseudogene</div>';else html += '<div class="text-left text-info font-weight-bold mb-1" title="Gene">Gene</div>';
  } else if (row.type == 1) html += '<div class="text-left text-danger font-weight-bold mb-1" title="Region">Region</div>';else return html;
...
function dosageSorter(one, two, row1, row2) {
  if (one === false || one === null || one == -5) return -1;
  if (two === false || two === null || two == -5) return 1;
...
function relationSorter(one, two, row1, row2) {
  if (row1.type == row2.type) {
    if (row1.locus_type == row2.locus_type) return row1.relation < row2.relation ? -1 : 1;else return row1.locus_type < row2.locus_type ? -1 : 1;
  } else return row1.type < row2.type ? -1 : 1;
  return 0;
}
...
function dateSorter(one, two, row1, row2) {
  ...
  if (row1rawdate < row2rawdate) return -1;else if (row1rawdate > row2rawdate) return 1;else return 0;
}

function locationSorter(one, two) {
  // there are some special genes with no defined coordinates
  if (one == null) return -1;else if (two == null) return 1;
...
function referenceSorter(one, two) {
  ...
  if (oneloc === null) return 0;
  if (oneloc[1] !== undefined) pmidone = oneloc[1];
  var twoloc = two.match(/PMID:\s*(\d+)/);
  if (twoloc[1] !== undefined) pmidtwo = twoloc[1];
  if (pmidone < pmidtwo) return -1;else if (pmidone > pmidtwo) return 1;
  return 0;
}
...
function ageSorter(one, two) {
  ...
  if (oneloc !== null && oneloc[1] !== undefined) {
    ...
    if (oneloc[2] == "Years") ageone *= 365;else if (oneloc[2] == "Months") ageone *= 30;else if (oneloc[2] == "Weeks") ageone *= 7;
  }
  ...
  if (twoloc !== null && twoloc[1] !== undefined) {
    ...
    if (twoloc[2] == "Years") agetwo *= 365;else if (twoloc[2] == "Months") agetwo *= 30;else if (twoloc[2] == "Weeks") agetwo *= 7;
  }

  if (ageone < agetwo) return -1;else if (ageone > agetwo) return 1;
  return 0;
}
