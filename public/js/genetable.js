...
function haplo2Formatter(index, row) {
  ...
  html += '<span class="pointer text-danger mt-2" data-toggle="tooltip" data-placement="top" title="' + row.haplo_history + '"><i class="fas fa-star ml-2 fa-lg"></i></span>';
  return html;
}
...
function triplo2Formatter(index, row) {
  ...
  html += '<span class="pointer text-danger mt-2" data-toggle="tooltip" data-placement="top" title="' + row.triplo_history + '"><i class="fas fa-star ml-2 fa-lg"></i></span>';
  return html;
}
...
function affiliateFormatter(index, row) {
  ...
  //return html;
  var html = '<a onclick="event.stopPropagation();" href="https://clinicalgenome.org/affiliation/' + row.agent + '" target="_gcep">' + row.label + '</a>' + '<div class="text-muted small">ClinGen Affliate ID: ' + row.agent + '</div>';
  return html;
}
...
function asbadgeFormatter(index, row) {
  ...
  if (row.animal_model_only) return '<a class="btn btn-default btn-block btn-classification" href="/kb/gene-validity/' + row.perm_id + '">' + '' + txt + '<div class="badge badge-warning">' + 'Animal Model Only</div></a>'; //return '<a class="btn btn-default btn-block btn-classification" href="/kb/gene-validity/' + row.perm_id + '">'
  //    + '' + txt + '</a>';

  return '<a class="btn btn-block text-white cg-table-' + txt + '" href="/kb/gene-validity/' + row.perm_id + '">' + '' + txt + '</a>';
}

function datebadgeFormatter(index, row) {
  return '<a class="btn btn-sm btn-outline-dark btn-block rounded-0" href="/kb/gene-validity/' + row.perm_id + '"><i class="far fa-file mr-2"></i> ' + '' + row.released + '</a>';
}
