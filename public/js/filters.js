...
$('.action-show-genes').on('click', function () {
  var viz = []; // check for legacy objects

  if ($(this).hasClass('btn')) return legacy_gene_switch($(this));
  ...
  if ($('.action-show-regions').prop('checked')) viz.push(1);
  filter_push("geneswitch", "type", viz);
  filter_process($table);
});
...
$('.action-show-regions').on('click', function () {
  ...
  if ($(this).hasClass('btn')) return legacy_region_switch($(this));
...
$('.action-show-overlap').on('click', function () {
  ...
  if ($('.action-show-contain').prop('checked')) viz.push("Contained");
...
$('.action-show-contain').on('click', function () {
  ...
  if ($('.action-show-overlap').prop('checked')) viz.push("Overlap");
  filter_push("container", "relationship", viz);
  filter_process($table);
});
...
$('.action-show-psuedo').on('click', function () {
  if ($(this).prop('checked')) {
    ...
    filter_push("pseudogene", "@filter", filterNotPseudos); //filter_pop("pseudogene");
  }
...
$('.action-show-hitsknown').on('click', function () {
  if ($(this).hasClass('fa-toggle-off')) {
    //$table.bootstrapTable('filterBy', {haplo_assertion: '3 (Sufficient Evidence)'});
    filter_push("hits", ["haplo_assertion", "triplo_assertion"], [3, 3]);
    $(this).removeClass('fa-toggle-off').addClass('fa-toggle-on');
...
module.exports = __webpack_require__(/*! /home/saatam/Projects/website-clinicalgenome-search/resources/js/filters.js */"./resources/js/filters.js");
