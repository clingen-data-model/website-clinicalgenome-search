...
$(function () {
  ...
  $('.action-toggle-pause').on('click', function () {
    ...
    var url = "/api/home/toggle-pause"; //submits to the form's action URL
  ...
  $("body").on('click', '.dropdown-menu li a', function () {
    ...
    if (typeof parent != 'undefined') {
      ...
      btngrp.find('.selection').text($(this).text()); // is this a gene or a disease block
  ...
  $('#unfollow_disease_form').validate({
    submitHandler: function submitHandler(form) {
      ...
      $.post(url, formData, function (response) {
        var url = "/api/home/follow/reload_disease";
        var disease = response.disease; //submits to the form's action URL
  ...
  $diseasetable.on('click', '.action-follow-disease', function (element) {
    ...
    swal({
      ...
      if (yes) {
        var curie = $(_this2).closest('tr').data('curie');
  ...
  $('.queryFindDisease').typeahead(null, {
    ...
    templates: {
      ...
      empty: ['<div class="tt-suggestion tt-selectable"><div class="list-group-item">Nothing found.</div></div>'],
  ...
  */
  ...
  $gencontable.on('click', '.action-remove-gc', function (element) {
    ...
    swal({
      ...
      if (yes) {
        var ident = $(_this3).closest('tr').data('uniqueid');
        $('#unfollow-gencon-field').val(ident);
        $('#unfollow_gencon_form').submit();
        var row = $(_this3).closest('tr').find('td:first-child').html();
    ...
    var dz = new Dropzone(el, {
      ...
      init: function init() {
        this.on("success", function (file, response) {
          ...
          var newcount = response.newcount; //submits to the form's action URL

          $.get(url, function (response) {
            //console.log(response.data);
            $('#gencon-table').bootstrapTable('load', response.data);
            $('#gencon-table').bootstrapTable("resetSearch", ""); // Check if any warnings to display
        ...
        this.on("addedfile", function (event) {// dtarget = id;
        });
      },
...
module.exports = __webpack_require__(/*! /home/saatam/Projects/website-clinicalgenome-search/resources/js/dashboard.js */"./resources/js/dashboard.js");
