        ...
        <div class="col-sm-9 border-left-4">
          <ul class="list-unstyled">
            ...
            @if (!empty($record->loss_pheno_ontology_id))
              ...
              @switch ($record->loss_pheno_ontology->value)
                @case ('Monarch')
                  <a target='external' href="{{env('CG_URL_MONARCH')}}{{ $record->loss_pheno_ontology_id }}" class="badge-info badge pointer ml-1">Monarch <i class="fas fa-external-link-alt"></i> </a>
                  @break
                @case ('OrhaNet')
                  <a target='external' href="{{env('CG_URL_ORPHANET')}}{{ $record->loss_pheno_ontology_id }}" class="badge-info badge pointer ml-1">OrphaNet <i class="fas fa-external-link-alt"></i> </a>
                  @break
                @case ('Disease Ontology')
                  <a target='external' href="{{env('CG_URL_DISEASEONTOLOGY')}}{{ $record->loss_pheno_ontology_id }}" class="badge-info badge pointer ml-1">Disease Ontology <i class="fas fa-external-link-alt"></i> </a>
                  @break
                @case ('MedGen')
                  <a target='external' href="{{env('CG_URL_MEDGEN')}}{{ $record->loss_pheno_ontology_id }}" class="badge-info badge pointer ml-1">MedGen <i class="fas fa-external-link-alt"></i> </a>
                  @break
              @endswitch
              </li>
            @endif
